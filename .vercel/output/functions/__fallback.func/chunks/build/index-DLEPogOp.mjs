...
import { d as ssrRenderAttrs_1, a as ssrRenderComponent_1, e as ssrRenderAttr_1, f as ssrRenderList_1, c as ssrRenderVNode, v as vueExports, g as ssrInterpolate_1, h as ssrRenderStyle_1, p as publicAssetsURL } from '../routes/renderer.mjs';
...
const _sfc_main$1 = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs_1(vueExports.mergeProps({ class: "relative home-banner w-full h-screen min-h-[600px] max-h-[1000px]" }, _attrs))}>`);
      if (banners.length > 0) {
        ...
        _push(ssrRenderComponent_1(_component_ClientOnly, null, {
          fallback: vueExports.withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<div class="absolute inset-0 overflow-hidden"${_scopeId}><img class="hero-slide-img h-full w-full object-cover scale-105"${ssrRenderAttr_1("src", _imports_0)} alt="NEBO Community"${_scopeId}><div class="absolute inset-0 bg-gradient-to-r from-brand-ink/95 via-brand-ink/60 to-transparent"${_scopeId}></div><div class="absolute inset-0 bg-gradient-to-t from-brand-ink/80 via-transparent to-brand-ink/30"${_scopeId}></div></div>`);
            } else {
              return [
                vueExports.createVNode("div", { class: "absolute inset-0 overflow-hidden" }, [
                  ...
                  vueExports.createVNode("div", { class: "absolute inset-0 bg-gradient-to-r from-brand-ink/95 via-brand-ink/60 to-transparent" }),
                  vueExports.createVNode("div", { class: "absolute inset-0 bg-gradient-to-t from-brand-ink/80 via-transparent to-brand-ink/30" })
                ])
              ];
            }
          })
        }, _parent));
        _push(`<div class="absolute inset-0 z-10 pointer-events-none"><div class="max-w-7xl mx-auto px-6 lg:px-8 h-full flex flex-col justify-end pb-32 md:pb-40"><div class="mb-4"><span class="inline-block text-brand-accent text-sm md:text-base font-semibold tracking-[0.25em] uppercase"> Yamaha NMAX NEO TURBO </span></div><h1 class="text-5xl sm:text-6xl lg:text-8xl font-black tracking-tight text-brand-text leading-[0.9] mb-6"> NEBO <span class="block text-brand-accent">INDONESIA</span></h1><p class="text-lg md:text-xl text-brand-muted/90 max-w-lg leading-relaxed mb-10"> Komunitas resmi pengguna Yamaha NMAX GEN TIGA <br><span class="text-brand-text font-medium">Ride together</span>, grow together! </p></div></div><div class="absolute bottom-10 right-6 lg:right-12 z-10 flex items-center gap-3"><span class="text-3xl md:text-4xl font-black text-brand-accent tabular-nums">${ssrInterpolate_1(String(activeIndex.value + 1).padStart(2, "0"))}</span><span class="text-brand-muted/50 text-lg">/</span><span class="text-sm text-brand-muted/50 tabular-nums">${ssrInterpolate_1(String(banners.length).padStart(2, "0"))}</span></div><div class="absolute bottom-6 left-1/2 -translate-x-1/2 z-10 hidden md:flex flex-col items-center gap-2 opacity-60"><div class="w-px h-10 bg-gradient-to-b from-transparent to-brand-accent animate-pulse"></div></div><div class="absolute left-6 lg:left-12 top-1/2 -translate-y-1/2 z-10 hidden lg:flex flex-col items-center gap-3"><div class="w-px h-16 bg-brand-accent/40"></div><span class="text-[10px] tracking-[0.3em] text-brand-muted/50 uppercase" style="${ssrRenderStyle_1({ "writing-mode": "vertical-lr" })}"> @nebo_idn </span><div class="w-px h-16 bg-brand-accent/40"></div></div></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div>`);

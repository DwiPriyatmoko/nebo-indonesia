...
import { v as vueExports, d as ssrRenderAttrs_1, e as ssrRenderAttr_1, f as ssrRenderList_1, a as ssrRenderComponent_1, g as ssrInterpolate_1, c as ssrRenderVNode } from '../routes/renderer.mjs';
import 'node:http';
import 'node:https';
import 'node:events';
import 'node:buffer';
...
const _sfc_main = {
  ...
  setup(__props) {
      ...
      social: [
        {
          ...
          icon: vueExports.defineComponent({
            render: () => vueExports.h("svg", { fill: "currentColor", viewBox: "0 0 24 24" }, [
              vueExports.h("path", {
                "fill-rule": "evenodd",
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      }, _attrs))}><h2 id="footer-heading" class="sr-only">Footer</h2><div class="mx-auto max-w-7xl px-6 pb-8 pt-16 sm:pt-24 lg:px-8 lg:pt-32"><div class="xl:grid xl:grid-cols-3 xl:gap-8"><div><img class="h-28 rounded-lg"${ssrRenderAttr_1("src", _imports_0)} alt="Company name"></div><div class="mt-16 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-2 xl:col-span-2 xl:mt-0"><div><h3 class="text-base font-bold leading-6 text-brand-text mb-6 border-b border-brand-border pb-2"> Tentang NEBO </h3><ul role="list" class="space-y-3"><!--[-->`);
      ssrRenderList_1(navigation.company, (item) => {
        _push(`<li>`);
        _push(ssrRenderComponent_1(_component_NuxtLink, {
          ...
          default: vueExports.withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<span class="w-2 h-2 bg-brand-accent rounded-full mr-3 opacity-0 group-hover:opacity-100 transition-opacity duration-200"${_scopeId}></span> ${ssrInterpolate_1(item.name)}`);
            } else {
              return [
                vueExports.createVNode("span", { class: "w-2 h-2 bg-brand-accent rounded-full mr-3 opacity-0 group-hover:opacity-100 transition-opacity duration-200" }),
      ...
      ssrRenderList_1(navigation.social, (item) => {
        _push(`<a${ssrRenderAttr_1("href", item.href)} target="_blank" rel="noopener noreferrer" class="text-brand-muted hover:text-brand-accent transition-colors duration-200 transform hover:scale-110"${ssrRenderAttr_1("title", item.name)}>`);
        ssrRenderVNode(_push, vueExports.createVNode(vueExports.resolveDynamicComponent(item.icon), {
          class: "h-6 w-6",
          "aria-hidden": "true"
        }, null), _parent);
        _push(`</a>`);
